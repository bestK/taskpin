        ...
        public:
            ...
            HRESULT STDMETHODCALLTYPE Invoke(HRESULT hr, ICoreWebView2Controller *ctrl) {
                ...
                if (m_wv->pending_url[0]) {
                    ...
                    /* For remote URLs: download and use NavigateToString */
                    if (wcsncmp(nav_url, L"http://", 7) == 0 || wcsncmp(nav_url, L"https://", 8) == 0) {
                        ...
                        if (data && out_len > 0) {
                            ...
                            if (is_html) {
                                WCHAR *whtml = (WCHAR *)malloc((out_len + 1) * sizeof(WCHAR));
                                if (whtml) {
                                    int wlen = MultiByteToWideChar(CP_UTF8, 0, data, out_len, whtml, out_len + 1);
                                    whtml[wlen] = L'\0';
                                    m_wv->webview->NavigateToString(whtml);
                                    free(whtml);
                                    free(data);
